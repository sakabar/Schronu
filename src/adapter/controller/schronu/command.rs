...
pub(super) fn parse_command(input: &str, mode: ParseMode) -> Result<Command, CommandParseError> {
    ...
    if mode == ParseMode::Interactive {
        match normalized.as_str() {
            ...
            "d" => {
                return Ok(Command::InteractiveShortcut(
                    InteractiveShortcut::NextMorning,
                ))
            }
            "w" => return Ok(Command::InteractiveShortcut(InteractiveShortcut::NextWeek)),
            "W" => {
                return Ok(Command::InteractiveShortcut(
                    InteractiveShortcut::DeferRoutine,
                ))
            }
            "y" => return Ok(Command::InteractiveShortcut(InteractiveShortcut::FiveYears)),
            _ => {}
        }
    }
    ...
    match name {
        ...
        "後" | "defer" if arguments.len() == 2 => {
            let amount = parse_i64(
                &arguments[0],
                "後",
                "amount",
                "整数で指定してください",
        ...
        _ => match known_command(name) {
            ...
            Some((kind, canonical_name)) => parse_known_action(kind, canonical_name, &arguments),
...
fn parse_known_action(
    ...
    let action = match kind {
        CommandKind::NewProject | CommandKind::HobbyProject | CommandKind::UnplannedProject => {
            ...
            let estimated_minutes = arguments
                .get(1)
                .map(|value| {
                    parse_i64(
                        value,
        ...
        CommandKind::Appointment | CommandKind::Start => CommandAction::TimeExpression {
            kind,
            canonical_name,
        ...
        CommandKind::Pick => {
            let task_id = required_argument(arguments, "選", "task_id", "選 <task_id>")?;
            CommandAction::Pick {
        ...
        CommandKind::Deadline | CommandKind::Category => CommandAction::StringValue {
            kind,
            canonical_name,
            value: required_argument(arguments, canonical_name, "value", "<command> <value>")?
                .to_string(),
        },
        ...
        CommandKind::Defer => CommandAction::TimeExpression {
            kind,
            canonical_name,
            values: arguments.to_vec(),
        },
        ...
        CommandKind::FocusHighest => {
            require_argument_count(arguments, 0, 0, canonical_name, "高")?;
        ...
        CommandKind::FocusLowest => {
            require_argument_count(arguments, 0, 1, canonical_name, "低 [days]")?;
        ...
        | CommandKind::DeferRoutines
        | CommandKind::Flatten
        | CommandKind::Pack => CommandAction::NoArguments {
            kind,
            canonical_name,
        },
        CommandKind::Noop
        ...
        | CommandKind::Arrange => unreachable!("handled before known action parsing"),
    };
