...
fn 全件pageはcliの予定segment順を500行境界でも維持する() {
    ...
    loop {
        ...
        leaf_flags.extend(
            page.rows
                ...
                .map(|row| (row.task.task_name.clone(), row.is_leaf)),
        );
    ...
    assert!(leaf_flags.contains(&("project 1".to_owned(), true)));
    assert!(leaf_flags.contains(&("project 2".to_owned(), false)));
    assert!(leaf_flags.contains(&("child".to_owned(), true)));
    assert_ne!(
        actual_names,
