...
fn test_make_obsidian_root_task_search_url_子タスクからrootのtask_idをqueryにする() {
    ...
    let _ = root_task.set_id(root_task_id);
...
fn test_execute_空_日付指定は指定日の予定開始時刻でtodoをpendingにする() {
    ...
    let _ = task.set_start_time(schronu_day_start + Duration::hours(4));
    let _ = task.set_estimated_work_seconds(30 * 60);
    let task_id = task.get_id().unwrap();
...
fn test_execute_集_日付指定はpendingを論理日開始へ集める() {
    ...
    let _ = task.set_pending_until(schronu_day_start + Duration::hours(6));
    let task_id = task.get_id().unwrap();
    let original_start_time = task.get_start_time().unwrap();
...
fn test_execute_空_日付指定はpending_untilの半開区間だけを変更する() {
    ...
    let _ = task.set_estimated_work_seconds(30 * 60);
    let _ = task.set_orig_status(Status::Pending);
    let _ = task.set_pending_until(schronu_day_start + Duration::hours(5));
    let task_id = task.get_id().unwrap();
    let original_start_time = task.get_start_time().unwrap();
...
fn test_execute_空_日付指定は予定候補外のpendingを変更しない() {
    ...
    let _ = task.set_start_time(schronu_day_start + Duration::days(1));
    let _ = task.set_estimated_work_seconds(30 * 60);
    let _ = task.set_orig_status(Status::Pending);
    let original_pending_until = schronu_day_start + Duration::hours(5);
    let _ = task.set_pending_until(original_pending_until);
    let task_id = task.get_id().unwrap();
...
fn test_execute_始と約の不正時刻はtask日時を変更しない() {
    ...
    for command in ["始 invalid", "約 invalid"] {
        ...
        let _ = task.set_start_time(original_start);
        let _ = task.set_deadline_time_opt(Some(original_deadline));
...
fn test_execute_集_2引数は従来通りtodoへ戻す() {
    ...
    let _ = task.set_start_time(now);
    let _ = task.set_orig_status(Status::Pending);
    let _ = task.set_pending_until(now + Duration::minutes(60));
    let task_id = task.get_id().unwrap();
...
fn test_execute_pack_前倒し内容と集計を表示する() {
    ...
    let _ = task.set_start_time(now);
    let _ = task.set_estimated_work_seconds(30 * 60);
    let _ = task.set_priority(9);
    let _ = task.set_pending_until(now + Duration::days(10));
    let _ = task.set_orig_status(Status::Pending);
    let task_id = task.get_id().unwrap();
...
fn test_execute_pack_収まらない候補はスキップ件数だけを表示する() {
    ...
    let _ = task.sync_clock(now);
    let _ = task.set_start_time(now);
    let _ = task.set_estimated_work_seconds(60 * 60);
    let _ = task.set_priority(9);
    let _ = task.set_pending_until(now + Duration::days(10));
    let _ = task.set_orig_status(Status::Pending);
    let repository = TestTaskRepository::new(task, now);
...
fn task_tree表示commandは製品経路で必ず1回flushする() {
    ...
    let _ = task.set_estimated_work_seconds(15 * 60);
    let _ = task.set_start_time(now);
    let task_id = task.get_id().unwrap();
    let commands = [
        "樹".to_string(),
...
fn test_execute_all_pendingタスクを予定時刻に含め_doneタスクを除外する() {
    ...
    let _ = pending_task.set_start_time(now);
    let _ = pending_task.sync_clock(now);
    let _ = pending_task.set_pending_until(now + Duration::hours(2));
    let _ = pending_task.set_orig_status(Status::Pending);
    ...
    let _ = done_task.set_start_time(now);
    let _ = done_task.sync_clock(now);
    let _ = done_task.set_orig_status(Status::Done);
...
fn task_listはtask名の表示幅境界を維持する() {
    ...
    let _ = width_70_task.set_estimated_work_seconds(30 * 60);
    let _ = width_70_task.set_start_time(now);
    let _ = width_70_task.set_pending_until(now);
    let _ = width_70_task.set_orig_status(Status::Pending);

    let width_71_name = "b".repeat(71);
    let width_71_task = new_test_task_handle(&width_71_name).unwrap();
    let _ = width_71_task.set_estimated_work_seconds(30 * 60);
    let _ = width_71_task.set_start_time(now);
    let _ = width_71_task.set_pending_until(now);
    let _ = width_71_task.set_orig_status(Status::Pending);
...
fn task_list検索はtyped_a_i列と名前の代表patternを製品経路で照合する() {
    ...
    let _ = task.set_project_category_opt(Some(ProjectCategory::Sustaining));
    let _ = task.set_estimated_work_seconds(30 * 60);
    let _ = task.set_start_time(now);
    let _ = task.set_pending_until(now);
    let _ = task.set_orig_status(Status::Pending);
...
fn task_list数値filterはtyped待ち属性のtaskを候補から除外する() {
    ...
    let _ = waiting_task.set_estimated_work_seconds(30 * 60);
    let _ = waiting_task.set_start_time(now);
    let _ = waiting_task.set_pending_until(now);
    let _ = waiting_task.set_orig_status(Status::Pending);
...
fn test_execute_all_締切順の予定時刻を表示する() {
    ...
    let _ = root_task.set_estimated_work_seconds(0);
...
fn test_execute_breakdown_子を順に作り締切を継承して最初の子へfocusする() {
    ...
    let _ = parent_task.sync_clock(now);
    let _ = parent_task.set_deadline_time_opt(Some(deadline));
...
fn test_execute_split_負数は親に残す時間として扱う() {
    let now = Local.with_ymd_and_hms(2026, 8, 11, 12, 0, 0).unwrap();
    let root = new_test_task_handle("root").unwrap();
    let _ = root.set_estimated_work_seconds(100 * 60);
...
fn test_execute_finish_繰り返しtaskの見積もりを実績との差に応じて補正する() {
    ...
    for (actual_work_seconds, expected_estimated_work_seconds) in cases {
        let parent_task = new_test_task_handle("繰り返しtask").unwrap();
        let _ = parent_task.set_repetition_interval_days_opt(Some(7));
        let _ = parent_task.set_estimated_work_seconds(600);
...
fn test_execute_repetition_数値だけの名前は拒否して元taskを変更しない() {
    ...
    let _ = task.set_estimated_work_seconds(45 * 60);
...
fn test_execute_actual_priority_work_不正値はfield付き入力エラーで状態を変更しない() {
    ...
    for command in ["実 invalid", "重 invalid", "働 invalid"] {
        let task = new_test_task_handle("更新対象").unwrap();
        let _ = task.set_actual_work_seconds(20 * 60);
        let _ = task.set_priority(7);
        let task_id = task.get_id().unwrap();
...
fn test_execute_deadline_不正日時はfield付き入力エラーを表示して状態を変更しない() {
    ...
    let _ = task.set_deadline_time_opt(Some(previous_deadline));
...
fn test_execute_finish_引数なしは実作業時間を自動加算して現在時刻で完了する() {
    ...
    let task = new_test_task_handle("タスク").unwrap();
    let _ = task.set_actual_work_seconds(60);
    let task_id = task.get_id().unwrap();
    let mut task_repository = TestTaskRepository::new(task.clone(), now);
    let mut free_time_manager = TestFreeTimeManager::default();
    let mut focused_task_id_opt = Some(task_id);
...
fn test_execute_flatten_過負荷日では葉より親を先に翌日へ延期する() {
    ...
    let _ = root.set_estimated_work_seconds(30 * 60);
    let _ = root.set_start_time(now);
    let _ = root.set_pending_until(now);
    let _ = root.set_orig_status(Status::Pending);
...
fn test_execute_flatten_未解消の超過が1分未満でも切り上げて表示する() {
    ...
    let _ = target.set_estimated_work_seconds(60 * 60 + 1);
...
fn test_execute_flatten_15分以下fragmentを避けたtaskを丸ごと翌論理日へ延期する() {
    ...
    let _ = fixed.set_deadline_time_opt(Some(
...
fn test_execute_flatten_終了時刻が期限と等しいtaskは延期できる() {
    ...
    let root = new_test_task_handle("平テスト").unwrap();
    let _ = root.set_estimated_work_seconds(0);
    let _ = root.set_deadline_time_opt(Some(
...
fn test_execute_flatten_延期対象自身の期限補正で翌日06時を維持できなければ延期しない() {
    ...
    let _ = target.set_deadline_time_opt(Some(
        try_logical_date_start(today + Duration::days(1)).unwrap() + Duration::minutes(30),
    ));
...
fn test_execute_flatten_待機taskと残作業0を延期候補から除外する() {
    ...
    let _ = waiting.set_is_on_other_side(true);
...
fn test_execute_flatten_35日後への退避で親の期限を超えるなら未解消として残す() {
    ...
    let _ = root.set_estimated_work_seconds(30 * 60);
    let _ = root.set_start_time(try_logical_date_start(boundary_date).unwrap());
    let _ = root.set_pending_until(try_logical_date_start(boundary_date).unwrap());
    let _ = root.set_orig_status(Status::Pending);
    let _ = root.set_deadline_time_opt(Some(
...
fn test_execute_flatten_未解消理由を固定順で表示して同じtaskを重複計上しない() {
    ...
    let _ = own_deadline.set_deadline_time_opt(Some(
        try_logical_date_start(today + Duration::days(1)).unwrap() + Duration::minutes(30),
    ));
...
fn test_execute_flatten_28日目は延期可能分を35日目へ退避して固定負荷を未解消にする() {
    ...
    let _ = deadline.set_deadline_time_opt(Some(boundary_start + Duration::hours(18)));
...
fn test_execute_calendar_余差59点5分以上は0時間60分で表示する() {
    ...
    let _ = task.set_estimated_work_seconds(7_799);
    let _ = task.set_start_time(now);
    let _ = task.set_pending_until(now);
    let _ = task.set_orig_status(Status::Pending);
...
fn test_execute_band_全日空き差分と繰り返し判定を帯へ反映する() {
    ...
    let _ = repetitive_group.set_estimated_work_seconds(0);
    let _ = repetitive_group.set_repetition_interval_days_opt(Some(7));
...
fn test_execute_show_all_年なし日付は完全日付と同じ予定を表示する() {
    ...
    let _ = task.set_start_time(scheduled_start);
    let _ = task.set_pending_until(scheduled_start);
    let _ = task.set_orig_status(Status::Pending);
...
fn test_cli_repository_transactionは外部更新を再読込してcommandを即時保存する() {
...

    {
        ...
        let _ = mcp_repository.sync_clock(now);
...
fn test_reload後にfocus中taskがdoneなら次候補を選び直す() {
    ...
    let _ = done.set_orig_status(Status::Done);
...
fn test_明示focus中に終了したらhandlerの次focusではなく元modeから再選択する() {
    ...
    let _ = automatic_task.set_orig_status(Status::Done);
...
fn test_interactive_commandによるfocus切替は次のrender時刻を開始時刻にする() {
    ...
    assert_eq!(focused_task_id_opt, Some(task_id));
    assert_eq!(last_focused_task_id_opt, Some(task_id));
    assert_eq!(focus_started_datetime, old_focus_started_datetime);
}
...
fn test_make_messages_about_focus_既存実績と表示中の作業時間から進捗を表示する() {
    ...
    let task = new_test_task_handle("タスク").unwrap();
    let _ = task.set_estimated_work_seconds(60 * 60);
    let _ = task.set_actual_work_seconds(10 * 60);
...
fn test_make_messages_about_focus_バーを1パーセント単位で表示する() {
    ...
    let now = Local.with_ymd_and_hms(2026, 7, 25, 12, 19, 0).unwrap();
    let task = new_test_task_handle("タスク").unwrap();
    let _ = task.set_estimated_work_seconds(100 * 60);
    let _ = task.set_actual_work_seconds(39 * 60);
...
fn test_make_messages_about_focus_見積時間超過時はバーだけ100パーセントを上限にする() {
    ...
    let task = new_test_task_handle("タスク").unwrap();
    let _ = task.set_estimated_work_seconds(100 * 60);
    let _ = task.set_actual_work_seconds(57 * 60);
...
fn test_make_messages_about_focus_見積時間が0なら進捗を未算定として表示する() {
    ...
    let now = Local.with_ymd_and_hms(2026, 7, 25, 12, 19, 0).unwrap();
    let task = new_test_task_handle("タスク").unwrap();
    let _ = task.set_estimated_work_seconds(0);
    let _ = task.set_actual_work_seconds(10 * 60);
...
fn test_render_focus_from_source_ancestor_error後もfocus詳細を描画してflushする() {
    ...
    let _ = render_focus_from_source(
        &mut writer,
...
fn test_render_interactive_screen_起動時と自動更新時の既定表示は帯() {
    ...
    let _ = render_interactive_screen(
        &mut stdout,
        &mut task_repository,
        &mut free_time_manager,
        FocusRenderState {
...
fn cliの外は開始時task名と経過時間を破棄journalへ記録する() {
    ...
    assert!(matches!(outcome, InteractiveRepositoryEventOutcome::CommandExecuted(CommandKind::Unfocus, _)));
    assert_eq!(repository.discarded_sessions.len(), 1);
    let event = &repository.discarded_sessions[0];
    assert_eq!(event.task_id(), task_id);
    assert_eq!(event.task_name_at_start(), "開始時task");
    assert_eq!(event.elapsed_seconds(), 75);
    assert_eq!(event.reason(), DiscardedSessionReason::CliUnfocus);
    assert_eq!(focus_started_datetime, operation_now);
}

#[test]
fn cliのtuckは製品submit経路でcli_tuck_awayを記録する() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
    let started_at = Local.with_ymd_and_hms(2026, 9, 10, 9, 0, 0).unwrap();
    let now = started_at + Duration::seconds(30);
    let task = new_test_task_handle("tuck task").unwrap();
    let task_id = task.get_id().unwrap();
    ...
    let mut last_focus = Some(task_id);
    ...
    assert!(matches!(
        outcome,
        InteractiveRepositoryEventOutcome::CommandExecuted(CommandKind::TuckAway, _)
    ));
    assert_eq!(repository.discarded_sessions.len(), 1);
    assert_eq!(
        repository.discarded_sessions[0].reason(),
...
#[test]
fn cliの明示的な別task_focusはcli_focus_switchを記録する() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
    let started_at = Local.with_ymd_and_hms(2026, 9, 10, 9, 0, 0).unwrap();
    let now = started_at + Duration::seconds(30);
    let root = new_test_task_handle("root").unwrap();
    let old = root.create_as_last_child(new_test_task_attr("切替前"));
    let next = root.create_as_last_child(new_test_task_attr("切替後"));
    ...
    let mut last_focus = Some(old_id);
    let mut focus_started = started_at;
    let mut mode = FocusSelectionMode::highest_priority();
    ...
    assert!(matches!(outcome, InteractiveRepositoryEventOutcome::CommandExecuted(CommandKind::Focus, _)));
    assert_eq!(focus, Some(next_id));
    assert_eq!(repository.discarded_sessions.len(), 1);
    assert_eq!(repository.discarded_sessions[0].task_id(), old_id);
    assert_eq!(repository.discarded_sessions[0].reason(), DiscardedSessionReason::CliFocusSwitch);
}

#[test]
fn focus切替commandの保存再試行は完全に同じevent_payloadを再利用する() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
    let started_at = Local.with_ymd_and_hms(2026, 9, 10, 9, 0, 0).unwrap();
    let first_attempt_at = started_at + Duration::seconds(75);
    let task = new_test_task_handle("開始時task").unwrap();
    let task_id = task.get_id().unwrap();
    let mut repository = TestTaskRepository::new(task, started_at)
        .with_storage_directory(&storage_dir.path);
    repository.highest_priority_leaf_task_id_opt = None;
    repository.save_failures_remaining.set(2);
    repository.save_failure_is_retryable = true;
    let mut free_time_manager = TestFreeTimeManager::default();
    let mut stdout = TestWriter::new();
    let mut focused_task_id_opt = Some(task_id);
    ...
    for attempted_at in [
        first_attempt_at,
        first_attempt_at + Duration::seconds(30),
    ] {
        let outcome = handle_interactive_submit_at(
            &mut stdout,
            &mut repository,
        ...
        assert!(matches!(outcome, InteractiveRepositoryEventOutcome::Retry(_)));
        assert_eq!(focused_task_id_opt, Some(task_id));
        assert_eq!(focus_started_datetime, started_at);
        assert!(repository.discarded_sessions.is_empty());
    }
    let pending = focus_selection_mode.pending_submit().unwrap().clone();
    ...
    let outcome = handle_interactive_submit_at(
        ...
        "外",
        first_attempt_at + Duration::minutes(1),
    );
    ...
    assert_eq!(repository.save_attempt_count.get(), 3);
    assert_eq!(repository.discarded_sessions.len(), 1);
    let event = &repository.discarded_sessions[0];
    assert_eq!(event.event_id(), pending.command_event_id);
    assert_eq!(event.started_at_epoch_ms(), started_at.timestamp_millis());
    assert_eq!(event.ended_at_epoch_ms(), first_attempt_at.timestamp_millis());
    assert_eq!(event.task_name_at_start(), "開始時task");
    assert_eq!(event.reason(), DiscardedSessionReason::CliUnfocus);
    assert_eq!(focus_started_datetime, first_attempt_at);
    assert!(focus_selection_mode.pending_submit().is_none());
}
...
fn cliの働はfocusが変わっても破棄journalへ記録しない() {
    ...
    let started_at = Local.with_ymd_and_hms(2026, 9, 10, 9, 0, 0).unwrap();
    let operation_now = started_at + Duration::seconds(75);
    let root = new_test_task_handle("作業task").unwrap();
    let task_id = root.get_id().unwrap();
    let mut repository = TestTaskRepository::new(root, started_at)
        .with_storage_directory(&storage_dir.path);
    repository.highest_priority_leaf_task_id_opt = None;
    ...
    assert!(matches!(outcome, InteractiveRepositoryEventOutcome::CommandExecuted(CommandKind::Work, _)));
    assert!(repository.discarded_sessions.is_empty());
}

#[test]
fn 捨commandは指定日のjournalを表示しsaveしない() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
    let operation_now = Local.with_ymd_and_hms(2026, 9, 10, 12, 0, 0).unwrap();
    let started_at = Local.with_ymd_and_hms(2026, 9, 9, 5, 59, 0).unwrap();
    let task = new_test_task_handle("集計対象").unwrap();
    let task_id = task.get_id().unwrap();
    let event = crate::entity::discarded_session::DiscardedSessionEvent::new(
        Uuid::new_v4(), task_id, "集計対象".to_string(), started_at,
        started_at + Duration::seconds(61),
        crate::entity::discarded_session::DiscardedSessionSource::Cli,
        DiscardedSessionReason::CliTuckAway,
    ).unwrap().unwrap();
    let revision_observer =
        seed_clean_task_revision_observer(&storage_dir.path, &task, operation_now);
    let mut repository = TestTaskRepository::new(task, operation_now)
        .with_storage_directory(&storage_dir.path)
        .with_pending_changes(false);
...
fn 捨commandの日付省略は06時境界の現在論理日を表示する() {
    ...
    let task_id = task.get_id().unwrap();
    let mut repository = TestTaskRepository::new(task, now).with_pending_changes(false);
    let mut free_time_manager = TestFreeTimeManager::default();
    let mut focus = Some(task_id);
    let mut stdout = TestWriter::new();
    ...
    let output = stdout.into_string();
    assert!(output.contains("破棄時間: 2026/09/09"));
    assert!(output.contains("合計: 00:00:00"));
    assert!(output.contains("記録はありません"));
}

#[test]
fn 正常なinteractive_exitだけが現在focusをjournalへ記録する() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
...
fn interactive_exitの再試行を別commandでcancelした場合は終了eventを残さない() {
    ...
    assert!(repository.discarded_sessions.is_empty());
    assert!(focus_selection_mode.pending_exit().is_none());
    assert!(String::from_utf8_lossy(&stdout.buffer).contains("合計: 00:00:00"));
}

#[test]
fn interactive_exitの再試行はidle_refreshを挟んでも同じpayloadを保持する() {
    let storage_dir = TestStorageDir::new();
    std::fs::create_dir_all(&storage_dir.path).unwrap();
    let started_at = Local::now() - Duration::seconds(5);
    let task = new_test_task_handle("終了時task").unwrap();
    ...
    assert!(matches!(first, InteractiveRepositoryEventOutcome::Retry(_)));
    let pending = focus_selection_mode.pending_exit().unwrap().clone();
    ...
    assert_eq!(focus_selection_mode.pending_exit(), Some(&pending));
    ...
    assert!(matches!(second, InteractiveRepositoryEventOutcome::Exit));
    assert_eq!(repository.discarded_sessions.len(), 1);
    assert_eq!(repository.discarded_sessions[0].event_id(), pending.exit_event_id);
    assert_eq!(
        repository.discarded_sessions[0].ended_at_epoch_ms(),
        pending.ended_at.timestamp_millis()
    );
}
