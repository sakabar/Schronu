...
#[cfg(unix)]
#[test]
fn test_delete_commitはprepare後に差し替えられたsymlink祖先を辿らない() {
    use std::os::unix::fs::symlink;

    let storage_dir = TestStorageDir::new();
    let external_dir = TestStorageDir::new();
    let project = storage_dir.path.join("project");
    let displaced_project = storage_dir.path.join("displaced-project");
    let target = project.join("old.yaml");
    let external_target = external_dir.path.join("old.yaml");
    fs::create_dir(&project).unwrap();
    fs::write(&target, b"internal").unwrap();
    fs::write(&external_target, b"external").unwrap();
    let prepared = prepare_with_directories_and_deletes(
        file_system_io(),
        &storage_dir.path,
        Uuid::from_u128(0x2261),
        &[],
        &[],
        &[target.as_path()],
    )
    .unwrap();
    fs::rename(&project, &displaced_project).unwrap();
    symlink(&external_dir.path, &project).unwrap();

    let actual = prepared.commit();

    assert!(actual.is_err());
    assert_eq!(fs::read(external_target).unwrap(), b"external");
    assert_eq!(
        fs::read(displaced_project.join("old.yaml")).unwrap(),
        b"internal"
    );
}

#[test]
fn test_prepare_deleteはrevision_fileを拒否する() {
    let storage_dir = TestStorageDir::new();
    let revision = storage_dir.path.join(".revision");

    let actual = prepare_with_directories_and_deletes(
        file_system_io(),
        &storage_dir.path,
        Uuid::from_u128(0x2262),
        &[],
        &[],
        &[revision.as_path()],
    );
    ...
    let error = match actual {
        ...
        Ok(prepared) => {
            prepared.discard().unwrap();
            panic!("revision delete target must fail");
        }
    };
    assert_eq!(
        error.operation,
