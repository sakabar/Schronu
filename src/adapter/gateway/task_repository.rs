...
mod tests {
    ...
    fn test_start_new_project_filesystemを変更しない() {
        ...
        task_repository
            .start_new_project(TaskHandle::new("filesystem非変更対象"))
            .unwrap();
    ...
    fn test_save_directory作成失敗を型付きerrorで返す() {
        ...
        task_repository.sync_clock(now);
        task_repository
            .start_new_project(TaskHandle::new("保存失敗対象"))
            .unwrap();
    ...
    fn test_save_project_yaml_read失敗でもatomic_writeを試す() {
        ...
        task_repository.sync_clock(now);
        task_repository
            .start_new_project(TaskHandle::new("read失敗対象"))
            .unwrap();
    ...
    fn test_save_load直後のprojectはcleanで新規projectだけを保存する() {
        ...
        source
            .start_new_project(TaskHandle::new("読込済み"))
            .unwrap();
        source.save().unwrap();
        ...
        repository
            .start_new_project(TaskHandle::new("新規"))
            .unwrap();
    ...
    fn test_load_revisionなしの既存storageを読める() {
        ...
        source.sync_clock(now);
        source
            .start_new_project(TaskHandle::new("既存project"))
            .unwrap();
        source.save().unwrap();
    ...
    fn test_save_actual_writeだけがrevisionを更新する() {
        ...
        repository.sync_clock(now);
        repository
            .start_new_project(TaskHandle::new("保存対象"))
            .unwrap();
