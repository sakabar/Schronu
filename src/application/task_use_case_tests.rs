...
#[test]
fn set_estimate_duration範囲外は属性とrevisionを変更しない() {
    let task = crate::test_support::new_task_handle("更新対象").unwrap();
    let task_id = task.get_id().unwrap();
    ...
    let actual = set_estimate(&mut repository, task_id, i64::MAX / 60);
    ...
    assert!(matches!(
        ...
        Err(ApplicationError::TaskTree(
            TaskTreeError::DeadlineCalculation {
                ...
                field: "estimated_work_seconds",
                source: crate::entity::datetime::DeadlineCalculationError::DurationOutOfRange {
