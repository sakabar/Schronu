...
/// fixed予約だけが原因で明示deadlineまでの容量を失うflexible候補を、予定計算上だけ
/// 必要最小限前倒しする。元fixedのwindow超過分と永続taskの日時は変更しない。
fn pull_forward_deadline_candidates_for_fixed_slots(
    prepared: &mut PreparedCandidates,
    last_synced_time: DateTime<Local>,
    instrumentation: &mut SchedulingInstrumentation,
) -> Result<(), SchedulingPolicyError> {
    for item in &mut prepared.pending {
        let SchedulingItem::Task(candidate) = item else {
            continue;
        };
        if prepared
            .total_work_seconds_by_id
        ...
        let Some(deadline) = candidate.deadline_time else {
            continue;
        };
        let remaining_seconds = candidate.remaining_seconds.max(0);
        if remaining_seconds == 0
            || deadline <= candidate.first_available_time
            || !fixed_slots_overlap(
                candidate.first_available_time,
                deadline,
                &prepared.occupied_fixed,
                instrumentation,
            )
        {
            continue;
        ...
        let adjusted = if candidate.atomic {
            if latest_contiguous_slot_start(
                candidate.id,
                candidate.first_available_time,
                deadline,
                remaining_seconds,
        ...
        } else if available_seconds_until(
            candidate.first_available_time,
            deadline,
            &prepared.occupied_fixed,
            instrumentation,
        ) >= remaining_seconds
        {
            ...
            Some(latest_cumulative_slot_start(
                candidate.id,
                last_synced_time,
                deadline,
                remaining_seconds,
        ...
        if let Some(adjusted) = adjusted {
            candidate.first_available_time = candidate.first_available_time.min(adjusted);
        }
    }
    Ok(())
}

fn fixed_slots_overlap(
    start: DateTime<Local>,
    end: DateTime<Local>,
    fixed_slots: &[(DateTime<Local>, DateTime<Local>)],
    instrumentation: &mut SchedulingInstrumentation,
) -> bool {
    fixed_slots.iter().any(|(fixed_start, fixed_end)| {
        instrumentation.record_occupied_slot_probe();
        *fixed_start < end && start < *fixed_end
    })
}

fn latest_contiguous_slot_start(
    task_id: Uuid,
    ...
    let work = Duration::try_seconds(work_seconds).ok_or(SchedulingPolicyError {
        task_id,
        start_time: deadline,
        work_seconds,
    })?;
    for (fixed_start, fixed_end) in fixed_slots.iter().rev() {
        instrumentation.record_occupied_slot_probe();
        if *fixed_start >= cursor {
            continue;
        }
        let gap_start = (*fixed_end).max(window_start);
        if *fixed_end < cursor && cursor.signed_duration_since(gap_start) >= work {
            return cursor
                .checked_sub_signed(work)
                .map(Some)
        ...
        cursor = cursor.min(*fixed_start);
        if cursor <= window_start {
            break;
        }
    }
    if cursor.signed_duration_since(window_start) < work {
        return Ok(None);
    }
    cursor
        .checked_sub_signed(work)
...
fn latest_cumulative_slot_start(
    task_id: Uuid,
    window_start: DateTime<Local>,
    deadline: DateTime<Local>,
    work_seconds: i64,
    ...
    let mut cursor = deadline;
    let mut remaining = work_seconds;
    for (fixed_start, fixed_end) in fixed_slots.iter().rev() {
        instrumentation.record_occupied_slot_probe();
        if *fixed_start >= cursor {
            continue;
        ...
        if *fixed_end < cursor {
            let gap_start = (*fixed_end).max(window_start);
            let available = cursor.signed_duration_since(gap_start).num_seconds();
            if available >= remaining {
                let work = Duration::try_seconds(remaining).ok_or(SchedulingPolicyError {
                    task_id,
                    start_time: cursor,
...
fn schedule_tasks_by_priority_instrumented(
    ...
    // 形で予約する。
    let mut prepared = classify_fixed_candidates(candidates, last_synced_time, instrumentation)?;
    pull_forward_deadline_candidates_for_fixed_slots(
        &mut prepared,
        last_synced_time,
        instrumentation,
    )?;
