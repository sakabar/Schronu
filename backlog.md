...
| TD-041 | P0 | 完了 | S | 小数秒付き現在時刻でslack indexとschedulerの論理時刻が乖離する |
...
### TD-041: 小数秒付き現在時刻でslack indexとschedulerの論理時刻が乖離する
...
- 検証: 障害ログと同じ718.250msの乖離を再現するRed testを追加し、修正後のrelease再選択、segment境界、整数作業秒数を固定した。rootとWeb serverの品質gateに成功した。
...
- debug buildでは次の候補選択時の不変条件検査で専用threadがpanicする。workerを再生成しない構成のため、server processが残っていても以後の「今」queryは成功しない。
...
- TD-040の再帰処理とstack使用量の改善は、本修正と独立した変更として扱う。
