...
| TD-058 | P2 | 未着手 | M | macOSショートカット操作の調査 | フォーカス解除時にSchronuが起動した時計アプリのタイマーだけを安全に削除できない |

### TD-058: フォーカス解除時の時計アプリタイマー削除

- 影響: `計`で起動したタイマーはフォーカスが変わっても残り、利用者が時計アプリで手動キャンセルする必要がある。
- 現状: macOSのショートカットには対象タイマーを識別して削除する確実な経路を確認できていない。画面操作の自動化は他のタイマーを誤って削除するおそれがある。
- 回避策: フォーカスを外した際は時計アプリの該当タイマーを手動でキャンセルする。Schronuの対話中の重複起動抑止は手動キャンセルを検知できない。
- 解消条件: 起動したタイマーの識別と削除をmacOSの安定した手段で検証し、他のタイマーを削除しない回帰テストを追加する。
