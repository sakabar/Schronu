...
日付別と全件一覧のtask名はCLIの「全」と同じ意味分類で、固定を淡青、繰返を青、単発を橙にします。締切は予定超過を赤、超過しない当日締切を黄、将来締切を緑にします。leaf taskは色とは別に太字と操作可否で示し、親taskにも種類色を表示します。この配色は一覧だけが対象で、CLIのicon・諦め候補色、セッションcard、dark modeは変更しません。
