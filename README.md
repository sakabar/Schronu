...
日付別一覧は、現在logical dateだけ現在時刻から先頭taskまで、およびtask間に1分以上の間隔があれば「N分間の空き時間」を表示します。最終task後は表示しません。「全て」では、隣接するtask日の間に予定のないlogical dateがあれば「N日間の空き時間」を表示し、空き日がなくてもlogical dateが変わる境界を横線で示します。どちらもtask名の検索中は空き時間行と境界線を隠し、検索をclearすると再表示します。
...
`全`は06:00を境界とするlogical dateごとに予定開始時刻の降順で表示します。隣り合うtaskのlogical dateが翌日に変わる場合はtask名欄の直前までの88マスの横線、2日以上離れる場合は88本の横線に続けて間に予定のない日数を「N日間の空き時間」と表示し、後ろを横線で埋めて全体を157マスに揃えます。いずれも後続taskの直前へ表示し、`全 <pattern>`で絞り込んだ場合も維持します。
