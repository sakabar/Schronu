...
task一覧は、標準出力が端末の場合にtask名を`帯`と同じANSI 256色で表示します。固定taskは淡青(110)、繰返taskは青(33)、単発taskは橙(208)です。予定上の締切超過`v`の締切列は赤(196)、今日締切だが間に合う`!`の締切列は黄(214)、明日以降の締切列は明緑(34)で表示します。この配色は`全`、`今`、`尾`とそれらのfilter表示に共通です。色は付加情報であり、既存の`【繰】`、`!`、`v`は維持します。パイプ・リダイレクト時はANSI escape sequenceを含まず、従来と同じA〜J列のプレーンテキストを出力します。
...
`暦`と`帯`の締切警告は、タスクごとの最終予定終了時刻が実際の締切を超えた場合に表示します。今日まで、明日、2日後から6日後までの3区分で、対象task数、最初の対象日、最大超過時間を要約し、予定上の締切超過(`v`)だけの確認には`全 v`を使用します。`全 〆`は、今日までに締切が来る`!`と予定上の締切超過`v`をまとめた締切注意一覧です。予定終了時刻と締切が同時刻なら警告しません。`暦`の`〆差`と`〆差比`は、締切日ごとの見積作業量とその日の容量を比較する参考値であり、この警告とは別の指標です。
