...
task一覧は、標準出力が端末の場合にtask名を`帯`と同じANSI 256色で表示します。固定taskは淡青(110)、繰返taskは青(33)、単発taskは橙(208)です。予定上の締切超過`v`のiconと締切列は赤(196)、今日締切だが間に合う`!`のiconと締切列は黄(214)、犠牲候補`A`のiconは濃紫(129)、明日以降の締切列は明緑(34)で表示します。通常の`-`と今日着手予定の`/`は無色です。この配色は`全`、`今`、`尾`とそれらのfilter表示に共通です。色は付加情報であり、既存の`【繰】`、`!`、`v`、`A`、`/`、`-`は維持します。パイプ・リダイレクト時はANSI escape sequenceを含まず、A〜J列のtask行と日付境界の非task行をプレーンテキストで出力します。`shell/copy_for_spreadsheet.sh`は非task行を無視するため、SpreadsheetのA〜J列定義は変わりません。
