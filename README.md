...
`全`は06:00を境界とするlogical dateごとに予定開始時刻の降順で表示します。隣り合うtaskのlogical dateが翌日に変わる場合は全列の横線、2日以上離れる場合は間に予定のない日数を「N日間の空き時間」として、それぞれ後続taskの直前へ表示します。この境界表示は`全 <pattern>`で絞り込んだ場合も維持します。
