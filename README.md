...
`暦`と`帯`の締切警告は、タスクごとの最終予定終了時刻が実際の締切を超えた場合に表示します。今日まで、明日、2日後から6日後までの3区分で、対象task数、最初の対象日、最大超過時間を要約し、全件の確認には`全 〆`を使用します。予定終了時刻と締切が同時刻なら警告しません。`暦`の`〆差`と`〆差比`は、締切日ごとの見積作業量とその日の容量を比較する参考値であり、この警告とは別の指標です。
