...
日付別一覧は、現在logical dateだけ現在時刻から先頭taskまで、およびtask間に1分以上の間隔があれば「N分間の空き時間」を表示します。最終task後は表示せず、task名の検索中は分単位の空き時間行を隠します。「全て」では、task名で絞り込んだ後の表示taskだけから日付差を再計算し、隣接するtask日の間に予定のないlogical dateがあれば「N日間の空き時間」、空き日がなくてもlogical dateが変わるときは横線を検索中も表示します。

日付別と全件一覧のtask名はCLIの「全」と同じ意味分類で、固定をANSI 256色127相当の濃いマゼンタ`#af00af`、繰返を青、単発を橙にします。締切は予定超過を赤、超過しない当日締切を黄、将来締切を緑にします。leaf taskは色とは別に太字と操作可否で示し、親taskにも種類色を表示します。この配色は一覧だけが対象で、CLIのicon・諦め候補色、セッションcard、dark modeは変更しません。

日付別と全件の検索は、前後空白を除いたUnicode小文字化によるtask名の部分一致です。全件検索はmemory内だけに保持し、日付別との往復と無効化後も維持しますが、reloadでは空に戻ります。全件一覧は検索後の先頭500行を表示し、日付区切りをtask件数に含めません。「さらに表示」で500行ずつ上限を増やすと、表示対象の先頭から日付区切りを再投影します。検索条件を変更またはclearすると表示上限を500行へ戻します。「全て」の表示中も日付入力を使用でき、妥当な日付を送信すると日付別一覧へ切り替わります。
...
- `計測を破棄して解除`: Web UIのセッションだけを削除し、実績を記録しない。
- `記録して解除`: click時刻で計測を停止し、セッション開始からclickまでの完了済み整数秒をtaskの実績へ加算してから、セッションを削除する。
- `計測を破棄して再開`: 今回のセッションでの経過時間を記録せず破棄し、同じtaskをclick時刻から再び計測する。taskの開始時見積と開始時実績は維持し、server通信は行わない。
...
記録と2種類の完了はセッション開始時の実績秒を期待値として照合します。「計測を破棄して完了」では開始時刻を実績計算に使わず、追加実績秒を0とします。他processが同じtaskを先に更新した場合、記録では従来どおり手動確認を求めます。2種類の完了では初回clickまでの計測とserverの最新実績をcardに表示し、「加算して完了」または「実績を維持して完了」で同じ開始・終了時刻のまま最新実績を期待値として再送できます。「計測を再開」すると、確認待ち時間を除外し、初回clickまでの計測を保った開始時刻と最新実績をlocalStorageへ保存してtimerを再開します。Schronu本体のcurrent taskは、自動セッション、一覧からの追加、解除、記録、完了のいずれでも変更しません。

終了操作のserver応答待ち中は対象cardの計測表示をclick時刻で停止し、対象をbuffer計算上の計測中セッションから除外します。完了の実績競合では、利用者が再完了または計測再開を選ぶまで初回click時刻で停止し続けます。それ以外の未commit確定errorでは計測表示とbuffer計算を自動的に元の進行状態へ戻し、transport切断またはrepository状態が不確実な場合は手動確認が完了するまで停止し続けます。持ち歩きロックは1.0秒の長押し後、封印対象操作をdispatchするたびに15秒の無操作期限を更新します。ただし、一時許可中に一覧または自動選定から最初のセッションが追加され、0件から1件になった場合は、追加成功後に即時再ロックします。再完了と計測再開も同じguardを通り、確認キャンセルでは期限を変更しません。
...
### フォーカス中のタスクのタイマーを起動する

macOSの対話モードで`計`または`timer`を入力すると、ショートカット`TimerForSchronu`を起動します。ショートカットへのテキスト入力は、見積秒数から記録済み実績秒数と今回のフォーカスで経過した完了済み整数秒を引いた残り秒数です。たとえば見積901秒、実績182秒、今回のフォーカス123秒なら`596`を渡します。残りが0秒以下、またはフォーカス中のタスクがない場合は起動せず、理由を表示します。実績とフォーカス状態は変更しません。

対話セッション中は、Schronuが起動したタイマーの予定終了時刻をタスクごとに記録します。同じタスクのタイマーが実行中と推定される間は再起動せず案内します。別タスクのタイマーが実行中なら、併走を案内して新たに起動します。記録はSchronuを終了すると消え、非対話実行にも引き継がれません。時計アプリで手動キャンセルした場合や、`open`の成功後にショートカット内で失敗した場合は検知できないため、表示は推定です。フォーカスを外しても時計アプリのタイマーは自動削除されません。

対話モードで`計 忘`または`timer forget`を入力すると、フォーカス中のタスクに対するSchronuのタイマー起動記録だけを消します。次の`計`は、その時点の残り秒数で通常どおり起動を判定します。記録がない場合や非対話実行ではその旨を表示します。時計アプリのタイマーは停止しないため、元のタイマーが残っていると2本動く可能性があります。残り秒数が0以下なら記録を消しても起動しません。
...
task一覧は、標準出力が端末の場合にtask名をANSI 256色で表示します。固定taskは濃いマゼンタ(127)、繰返taskは青(33)、単発taskは橙(208)です。予定上の締切超過`v`のiconと締切列は赤(196)、今日締切だが間に合う`!`のiconと締切列は黄(214)、犠牲候補`A`のiconは濃紫(129)、明日以降の締切列は明緑(34)で表示します。通常の`-`と今日着手予定の`/`は無色です。この配色は`全`、`今`、`尾`とそれらのfilter表示に共通です。色は付加情報であり、既存の`【繰】`、`!`、`v`、`A`、`/`、`-`は維持します。パイプ・リダイレクト時はANSI escape sequenceを含まず、A〜J列のtask行と日付境界の非task行をプレーンテキストで出力します。`shell/copy_for_spreadsheet.sh`は非task行を無視するため、SpreadsheetのA〜J列定義は変わりません。
...
`暦`と`帯`の締切警告は、タスクごとの最終予定終了時刻が実際の締切を超えた場合に表示します。今日まで、明日、2日後から6日後までの3区分で、対象task数、最初の対象日、最大超過時間を要約し、予定上の締切超過(`v`)だけの確認には`全 超`を使用します。`全 v`も互換aliasとして使用できます。`全 〆`は、今日までに締切が来る`!`と予定上の締切超過`v`をまとめた締切注意一覧です。予定終了時刻と締切が同時刻なら警告しません。`暦`の`〆差`と`〆差比`は、締切日ごとの見積作業量とその日の容量を比較する参考値であり、この警告とは別の指標です。

終了予定時刻の警告も、今日、明日、2日後から6日後までのローリング7日間を対象にします。今日と明日は当日の超過時間を表示し、週次警告は累積の空き時間を超過した日数、最初の超過日、期間内の最大超過時間を要約します。今日の候補は`尾`、明日は`尾 明`、週次は`尾 週`で確認し、週次の調整では7日後以降を延期先にします。容量と見積作業量が等しい日は警告しません。
...
```text
# 利用不可  x 経過済み  = 繰返  - 単発  : 余差  . 空き  > 超過  (1文字=15分)
```

色は順に、利用不可が淡青(110)、経過済みが灰(244)、繰返が青(33)、単発が橙(208)、余差が暗緑(28)、空きが明緑(34)、超過が赤(196)です。標準出力が端末の場合にだけANSIエスケープシーケンスを含め、パイプ・リダイレクト時は無色で出力します。

`#` は24時間からその日の全日空き時間を引いた利用不可時間です。`x` は当日だけに表示され、同日の全日空き時間から現在以降の空き時間を引いた、既に経過した空き時間を表します。`:` はrho 0.7に対する当日の余裕です。
...
標準出力の受け手が先に終了した場合(`BrokenPipe`)は、対話・非対話のどちらでもパイプライン利用時の正常終了として扱います。それ以外のterminal出力失敗とraw mode初期化失敗は、原因を保持したI/O errorとして非0で終了します。対話モードでraw modeの開始に成功した後は、通常終了、command処理失敗、terminal出力失敗のいずれでも終了時にterminal状態を復元します。

対話モードの更新commandは、成功するたびにrepository transaction内で保存されます。その後にterminal出力が失敗しても追加保存は行わず、まだ送信していない入力だけを破棄します。raw mode初期化失敗でも追加保存は行いません。Ctrl-D、標準入力の切断、標準入力の読込失敗では、従来どおりrepositoryを再読込してから保存を試みます。
...
* `!` : 現在のlogical dateが締切であり、タスクの完了予定時刻が締切以前である
* `v` : 締切のlogical dateにかかわらず、タスクの完了予定時刻が締切を過ぎている
...
締切と完了予定時刻が同じlogical dateにある場合は時間差を、異なるlogical dateにある場合は日付差を表示します。logical dateは06:00に切り替わります。

* `____/__/__` : 締切なし
* `____-HH:MM` : 同じlogical date内で、完了予定時刻は締切のHH時間MM分前である
  * (例) `____-00:30` : 完了予定時刻は締切の30分前である
* `+HH:MM____` : 同じlogical date内で、完了予定時刻は締切をHH時間MM分超過している
  * (例) `+01:30____` : 完了予定時刻は締切を1時間30分超過している
* `+HH:MMASAP` : 締切が既に最終同期時刻より前であり、完了予定時刻は締切をHH時間MM分超過している
* `_____-001D`など : 完了予定のlogical dateは締切のlogical dateよりDDD日前である
  * (例) `_____-001D` : 完了予定のlogical dateは締切のlogical dateより1日前である
* `_____+001D`など : 完了予定のlogical dateは締切のlogical dateよりDDD日後である
  * (例) `_____+002D` : 完了予定のlogical dateは締切のlogical dateより2日後である

`DDD`は最低3桁で表示し、1000日以上の場合は桁数を増やします。締切と完了予定時刻が完全に一致する場合は`____-00:00`です。
...
`約`または`appointment`で開始時刻を設定したタスクは`fixed_start: true`かつ`atomic: true`の予定となり、priorityや締切にかかわらず指定時刻から分割せずに実行します。`始`または`start`で開始時刻を設定し直すと`fixed_start`だけを解除してflexibleへ戻り、`atomic`は保持します。fixed予定同士が重なる場合は重複したまま表示し、その他のタスクはその予約時間を避けます。

ただし、自身に`repetition_interval_days`を持つ繰り返し親taskでは、`fixed_start`と`atomic`は次回子taskを生成するための既定値です。繰り返し親task自身はfixed予定にならず、新しく生成される子だけが親の値を継承します。繰り返し親の直接の子に`約`を使うと、その子と親の両方の`fixed_start`と`atomic`がtrueになり、次回以降の子も両属性を継承します。既存の他の子は変更しません。個別の子に対する`始`は親の値を変更しません。
