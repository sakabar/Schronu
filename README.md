...
明示締切を持つflexible taskは、元の着手可能時刻から締切までの枠をfixed予約が塞いでいる場合、予定計算上の着手可能時刻だけを必要最小限前倒しします。非`atomic`はfixedを除く累積容量、`atomic`は連続した空き枠で判定し、保存済みの開始時刻や先送り時刻は変更しません。現在以降に全量を確保できない非`atomic`は現在まで前倒しして利用可能な枠を使い、連続枠を確保できない`atomic`は通常どおり配置します。いずれも残る締切超過は`v`で表示します。
