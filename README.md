...
一覧のleaf taskには「セッション」と「先送り」を表示します。46rem以下では「＋/✓」と「→」で表示します。deadlineがないtaskと〆切logical dateが現在より未来のtaskは1tapで先送りします。〆切logical dateが現在以前なら、task名、締切日時、当日または超過の状態を行内で確認し、「先送りする」を押した時だけ送信します。現在logical dateは毎秒更新するbrowser時刻から算出するため、06:00を跨いでpageを開いたままでも確認条件が切り替わります。Web UIはbrowserとserverが同じtimezoneで使われることを前提とします。logical dateは06:00境界で、日次容量用の`logical_date_end`はこの確認判定に使いません。通常taskと、〆切のlogical dateが現在より未来にあるルーチンtaskは、CLIの`d`と同じく次の論理日開始(既定06:00)を指定してPendingにします。ただしdeadlineを持つtaskでは、既存方針により`pending_until`が`deadline - 見積時間 - 5分`まで前倒しされ、deadlineが迫っていれば実効statusがTodoになる場合があります。親の反復日数と自身のdeadlineを持ち、現在のlogical dateが〆切日以降に達したルーチンtaskだけを、CLIの`W`と同じ次周期へ移動します。実行中セッションと同じtaskは先送りできません。成功後は選択中の一覧を再取得し、検索と日付入力は維持します。
