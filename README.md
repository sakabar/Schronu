...
一覧のleaf taskには「セッション」と「先送り」を表示します。46rem以下では「＋/✓」と「→」で表示します。通常taskはCLIの`d`と同じ次の論理日開始(既定06:00)までPendingにし、親の反復日数と自身のdeadlineを持つルーチンtaskはCLIの`W`と同じ次周期へ移動します。実行中セッションと同じtaskは先送りできません。成功後は選択中の一覧を再取得し、検索と日付入力は維持します。
