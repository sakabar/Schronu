...
`copy_for_spreadsheet.sh`は、06:00を業務日の境界として、入力内で新しい業務日へ移った最初のタスクのP列へ睡眠時間420分を算入します。00:00から05:59開始のタスクは前の業務日として扱います。コピー対象の先頭行には算入しません。G列の見積もり分数は変更せず、P列の予定完了時刻の計算だけに420分を加えます。R列が`W`または`d`の行は、タスクの見積もりと睡眠時間のどちらもP列へ加算しません。
