...
一覧のleaf taskには「セッション」と「先送り」を表示します。46rem以下では「＋/✓」と「→」で表示します。通常taskと、〆切のlogical dateが現在より未来にあるルーチンtaskは、CLIの`d`と同じく次の論理日開始(既定06:00)を指定してPendingにします。ただしdeadlineを持つtaskでは、既存方針により`deadline - 見積時間 - 5分`まで`pending_until`が前倒しされ、deadlineが迫っていれば実効statusがTodoになる場合があります。親の反復日数と自身のdeadlineを持ち、現在のlogical dateが〆切日以降に達したルーチンtaskだけを、CLIの`W`と同じ次周期へ移動します。実行中セッションと同じtaskは先送りできません。成功後は選択中の一覧を再取得し、検索と日付入力は維持します。
