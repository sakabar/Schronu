...
候補は優先度が高い順です。同じ優先度では現在の予定日時が早い順、予定日時も同じ場合はUUIDの昇順に扱います。タスクの残作業時間が丸ごと収まる最初の日へ配置し、複数日の余差は合算しません。最優先候補が7日間のどの日にも収まらない場合はそこで終了し、より低優先度のタスクには進みません。`atomic`タスクは、実際の連続した空き枠にも全量が収まる場合だけ前倒しします。
