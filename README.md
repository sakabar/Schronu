...
3引数の`空|clear <時刻> <月/日>`と`集|gather <時刻> <月/日>`は、指定日の予定表に含まれる未完了の葉タスクだけを対象にします。予定が分割されている場合も、指定日のセグメントを1つでも持つタスクを対象にします。
...
`<月/日>`が過去なら翌年として解釈します。Schronu業務日は06:00開始です。指定日の06:00より前の時刻は翌暦日の時刻として扱い、`24:00`以降も受け付けます。たとえば`8/15`の`03:00`は8月16日03:00、`24:00`は8月16日00:00です。不正な時刻や、06:00ちょうどのような空区間は何も変更しません。
