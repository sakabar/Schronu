...
`平`は、明日から28日以内の日を順に調べ、日次の確保時間から既存の予定を引いた空きへ収まるタスクを延期します。`busy_time_slot`で表される毎週定期的な行動不能時間も確保時間へ反映されます。

タスクは分割せず、残作業時間の全体が空きへ収まる場合だけ移動します。空きへ収まらないタスクは飛ばして別のタスクを探し、移動するたびに予定を再計算します。移動件数に固定上限はありませんが、同じ実行中に同じタスクを複数回延期することはありません。延期によって期限を超えるタスク、待っていることを表すタスク、残作業時間が0のタスクは対象外です。

移動結果の各行は`平 <延期元> <延期先> <見積時間> <優先度> <task ID> <task名>`の列順で表示します。延期元は、各taskを移動する直前の最新予定日です。
