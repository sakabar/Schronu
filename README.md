...
日付別一覧は、現在logical dateだけ現在時刻から先頭taskまで、およびtask間に1分以上の間隔があれば「N分間の空き時間」を表示します。最終task後は表示しません。「全て」では、隣接するtask日の間に予定のないlogical dateがあれば「N日間の空き時間」を表示します。どちらもtask名の検索中は空き時間行を隠し、検索をclearすると再表示します。
