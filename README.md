...
終了操作のserver応答待ち中は対象cardの計測表示をclick時刻で停止し、対象をbuffer計算上の計測中セッションから除外します。完了の実績競合では、利用者が再完了または計測再開を選ぶまで初回click時刻で停止し続けます。それ以外の未commit確定errorでは計測表示とbuffer計算を自動的に元の進行状態へ戻し、transport切断またはrepository状態が不確実な場合は手動確認が完了するまで停止し続けます。持ち歩きロック中は競合を発生させた完了操作で一時許可を消費済みのため、再完了と計測再開のそれぞれに新たな1操作許可が必要です。
