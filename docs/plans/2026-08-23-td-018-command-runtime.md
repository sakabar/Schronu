...
| 38 | `Test: Task tree contextをwriter非依存で固定する` | `TaskTreeCommandContext`のchildren/deepestを`Result<Option<DisplayModel>, ApplicationError>`、next-upを`Result<NextUpResult, ApplicationError>`へ変えるRed contract。`NextUpResult::NoDisplay`と`ReportedError(ApplicationError)`で成功時の無表示とhandler報告対象errorを情報損失なく区別し、tree順、focus mutation、外側へ伝播する`ApplicationError`を保持する | 37 | handler、TaskTree context、tree/focus contract | writer-free trait interface欠如の1理由でRedを確認してtestだけcommit |
| 39 | `CLI: Task tree context表示をtyped modelへ移す` | TaskTree contextからwriterとANSI capabilityを除く。children/deepestの`Option<DisplayModel>`をhandler outcomeへcomposeし、next-upの`NoDisplay`をsemanticな空`Sequence`、`ReportedError`をhandler所有のError messageへ変換する。focus mutation、既存error文言、外側へ伝播する`ApplicationError`を維持 | 38 | handler、command context、tree/focus runtime | 対象test後に全品質ゲート |
