...
| 38 | `Test: Task tree contextをwriter非依存で固定する` | `TaskTreeCommandContext`のfocus children/deepest/next-upをtyped `DisplayModel` returnへ変え、既存tree・message順、focus mutation、next-upのreported semantic errorと伝播`ApplicationError`を保持するRed contract | 37 | handler、TaskTree context、tree/focus contract | writer-free trait interface欠如の1理由でRedを確認してtestだけcommit |
| 39 | `CLI: Task tree context出力をtyped modelへ移す` | TaskTree contextからwriterを除き、handlerがtyped tree/messageを`CommandOutcome`へ順序どおりcomposeする。focus mutation、reported semantic error、伝播`ApplicationError`を維持 | 38 | handler、command context、tree/focus runtime | 対象test後に全品質ゲート |
