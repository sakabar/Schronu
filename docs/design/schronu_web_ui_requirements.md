...
- **REQ-NET-001**: server通信を初回`bootstrap`、一覧の日付選択による`list_tasks`、`自動セッション`、`計測を破棄して解除`の`discard_session`、`記録して解除`、2種類の完了確定、完了実績競合の再完了、集計の日付選択による`list_discarded_sessions`、および4種類のセッション終了成功後と集計で日付変更後に一覧へ戻る場合の必要な`list_tasks`に限定すること。
...
| AC-007 | 初回、一覧・集計の日付選択、自動セッション、破棄解除、記録、2種類の完了確定、完了競合再送、4種類のセッション終了成功後の一覧再取得、および集計で日付変更後に一覧へ戻るための必要な再取得だけが仕様どおりのserver requestを発生させる。 |
