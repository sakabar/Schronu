...
| 睡眠時間 | Web表示用の基準睡眠時間420分へbufferを加えた値。bufferが負なら、加算後の値が0以上でも「基準より不足」の文言と赤色で示す。 |
...
- **REQ-BUFFER-004**: bufferが負の場合は、加算後の睡眠時間が0以上でも表示を赤い文字にし、可視ラベルとARIA labelの両方で「基準より不足」と示すこと。bufferが0以上の場合は通常色にし、不足文言を表示しないこと。
