...
- **REQ-LIST-018**: `全て`は初期表示・reload・tab切替では取得せず、button押下時にだけ完了済みを除く全タスクを500件ずつ最後まで取得すること。全ページ成功後にだけ検索・表示を有効にし、失敗時は部分一覧を表示せず再試行を提供すること。親と保留中も1タスク1行で含め、予定列には最初の計算上の予定logical dateを`YYYY/MM/DD(曜日)`、予定なしには`—`を表示すること。着手可能な末端だけにセッション追加を表示し、全件側では先送りを表示しないこと。検索は日付別と同じ条件で取得済み全件にclient内で適用し、表示は500行ずつ増やすこと。全件と全件側の検索文字列はmemoryだけに保持し、task更新後は破棄すること。既存の曜日button列とtask名cellの横scrollは保ち、page全体の横はみ出しを増やさないこと。
