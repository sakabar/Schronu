...
- **REQ-SESSION-008**: セッションtab表示中にセッション終了操作またはrepository確認によってセッション件数が実際に減少して0件になった場合は、一覧tabへ切り替えること。削除失敗、件数不変、1件以上残る場合、一覧、発火履歴、集計tabの表示中はtabを変更しないこと。
...
- **REQ-NET-001**: server通信を初回`bootstrap`、一覧の日付選択による`list_tasks`、`自動セッション`、`計測を破棄して解除`の`discard_session`、`記録して解除`、2種類の完了確定、完了実績競合の再完了、集計tabへの切替または集計の日付選択による`list_discarded_sessions`、および4種類のセッション終了成功後と集計で日付変更後に一覧へ戻る場合の必要な`list_tasks`に限定すること。
...
- **REQ-NET-008**: 利用者起点のserver通信ではrequest開始からresponse適用まで画面全体に待機表示を出し、背面操作を無効にすること。SSR初期HTMLとhydration前の表示は全面overlayを出さず「画面を復元しています…」を表示すること。localStorage復元後の`bootstrap`と保存日付の`list_tasks`は背景更新とし、保存一覧があれば「前回の表示です。最新状態を確認中…」、なければ「最新状態を確認中…」を表示すること。この背景更新statusは全tab共通でbottom navigation直上に浮遊表示し、表示・消去で本文の位置を変えないこと。背景更新中はlocal操作を許可し、日付選択・送信、自動セッション、記録・完了・競合再送、「計測を破棄して解除」はUIとreducerの両方で拒否すること。失敗時は保存一覧を維持して同じ浮遊表示に再試行buttonを表示すること。
...
- **REQ-LOCK-003**: ロック中は「自動セッション」、一覧からの「セッション」追加、「計測を破棄して解除」、「記録して解除」、「計測を破棄して完了」、「記録して完了」、完了実績競合の再完了・計測再開、「repository確認済み」の変更操作をclientの共通guardで遮断すること。
...
- **REQ-SUMMARY-003**: 集計状態を未選択、取得中、取得成功、取得失敗として区別すること。別日の取得開始時は旧日の集計を表示せず、取得失敗時は取得中表示を残さず、集計内にerrorと同じ日付を再試行するbuttonを表示すること。集計内errorを共通error bannerへ重複表示しないこと。日付buttonは一覧と選択状態を共有し、mobileでも横scroll可能であること。背景更新または通常通信中は日付buttonと再試行buttonをdisabledにし、clientからの重複dispatchも拒否すること。
...
| ID | 受入条件 |
| --- | --- |
| AC-001 | viewport下端に「セッション」「一覧」「発火履歴」「集計」の4つの固定tabが表示され、desktopで44px以上、46rem以下で40px以上の均等幅button、safe area、本文との非重複、選択indicatorと`aria-pressed`を維持し、利用者向け文言に「フォーカス」が残っていない。 |
...
| AC-007 | 初回、一覧・集計の日付選択、集計tabへの切替、自動セッション、破棄解除、記録、2種類の完了確定、完了競合再送、4種類のセッション終了成功後の一覧再取得、および集計で日付変更後に一覧へ戻るための必要な再取得だけが仕様どおりのserver requestを発生させる。 |
...
| AC-022 | 利用者起点の一覧・集計取得、自動選定、破棄解除、記録、2種類の完了の各server通信中は全画面の「通信中…」とスピナーが表示され、背面を操作できない。reload直後の背景更新では全面overlayを出さず、前回表示または未取得状態と更新statusを示す。複数の通常通信は最後のresponseまでoverlayを維持し、成功と各error応答の完了後に解除される。 |
...
| AC-024 | セッションtabで最後のセッションを正常に削除すると一覧tabへ移り、複数セッション中の1件削除、server失敗、localStorage削除失敗、完了競合、計測再開、一覧・発火履歴・集計tab表示中の削除では強制遷移しない。集計取得と、集計で日付変更後に一覧へ戻るための必要な再取得以外にtab遷移由来のserver通信を追加しない。 |
