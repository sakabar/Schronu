...
- **REQ-LIST-002**: 日付buttonの左に`全て`buttonを置き、日付buttonの先頭を`曜 今日`、2番目を`曜 明日`、3番目以降を曜日で表示すること。
...
- **REQ-LIST-018**: `全て`は初期表示・reload・tab切替では取得せず、button押下時にだけCLI `全`の無指定表示と同じ`get_schedule`の実task予定segmentを500行ずつ最後まで取得すること。CLI専用の空き時間行・要約は含めず、Web独自の対象抽出・状態filter・日付sortを行わないこと。CLI表示は逆順、Web表示は反転前の直近segmentが先頭となる順序とし、同一taskの複数segmentは別行、予定segmentのないtaskは対象外とすること。各予定列は共通処理で得た開始時刻のlogical dateを`YYYY/MM/DD(曜日)`で表示すること。全ページ成功後にだけ検索・表示を有効にし、失敗時は部分一覧を表示せず再試行を提供すること。日付別と同じschedule rank 0の葉taskだけにstatusを問わずセッション追加を表示し、全件側では先送りを表示しないこと。検索は日付別と同じ条件で取得済み全行にclient内で適用し、表示は500行ずつ増やすこと。全件と全件側の検索文字列はmemoryだけに保持し、task更新後は破棄すること。既存の曜日button列とtask名cellの横scrollは保ち、page全体の横はみ出しを増やさないこと。

### 4.8 通信制限と発火履歴

- **REQ-NET-001**: server通信を初回`bootstrap`、日付選択、`全て`押下後の全件page取得、`自動セッション`、一覧の「先送り」、`記録して解除`、`計測を破棄して完了`の確定、`記録して完了`、完了実績競合の再完了、および各mutation成功後の`list_tasks`に限定すること。
- **REQ-NET-002**: tab切替、毎秒tick、一覧検索の入力・clear、一覧の「セッション」と追加成功後の検索解除・tab切替、先送りの確認表示とキャンセル、「計測を破棄して再開」、`計測を破棄して完了`の確認表示とキャンセルではserver通信を行わないこと。
