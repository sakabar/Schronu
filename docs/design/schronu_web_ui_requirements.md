...
- **REQ-LIST-017**: deadlineがないtaskと〆切logical dateが現在より未来のtaskは確認なしで先送りし、〆切logical dateが現在以前のtaskはtask名、締切日時、当日または超過の状態を行内に表示して確定後だけ先送りすること。clientの現在logical dateは毎秒更新するbrowser時刻から求め、06:00を跨いでpageを開いたままでも更新すること。確認表示とキャンセルではserver通信を行わず、日時をlogical dateへ変換できない場合も安全側へ倒して確認すること。通常taskと〆切logical dateが現在より未来にあるルーチンtaskは、次の論理日開始を指定してPendingにすること。ただしdeadlineを持つtaskでは既存のdeadline policyに従い、`pending_until`を`deadline - 見積時間 - 5分`まで前倒しし、deadline接近時は実効statusをTodoにしてよい。自身にdeadlineがあり直接の親に反復日数があるtaskのうち、現在logical dateが〆切logical date以降に達したtaskだけは、親の反復日数だけdeadlineとstart timeを移動すること。logical dateは06:00を境界とし、日次容量用の`logical_date_end`は確認判定に使わないこと。Web UIはbrowserとserverが同じtimezoneで使われることを前提とする。セッション中の同一task、rank非0、通信操作禁止中、mutation safetyによる全体停止中、持ち歩きロック中はbuttonと確定操作を無効化して実行せず、成功応答時に同一taskの全segmentを表示一覧から除去してから選択日の一覧を再取得し、検索と日付入力を維持すること。一覧再取得に失敗しても除去したsegmentを復活させないこと。
