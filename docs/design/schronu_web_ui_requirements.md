...
- **REQ-ACTION-001**: 各cardに「計測を破棄して再開」「計測を破棄して解除」「記録して解除」「計測を破棄して完了」「記録して完了」の5buttonを表示すること。通常幅では再開buttonを全幅の最上段、既存4buttonをその下の2列とし、34rem以下では再開buttonを先頭とする1列にすること。
...
| AC-015 | 各cardに5操作が表示され、計測を破棄して再開は先頭で経過秒だけを破棄し、計測を破棄して完了はcard内の確認を経た確定時だけ1回送信され、キャンセルでは送信されない。3終了操作はclick時刻でcardの計測を停止し、通信待ちで表示や実績を増やさない。2種類の完了は`record_elapsed_seconds`の真偽を含む発火履歴で区別される。 |
