...
このkeyが存在しない場合、またはversion 1の`mutation_blocked`が`false`の場合だけ全体blockを解除する。`committed_task_ids`はserver commit成功後にlocal session削除だけが失敗したtask UUIDであり、`mutation_blocked: true`と同じtask UUIDの妥当な`fixed_requests` entryを必須とする。`fixed_requests`はtask UUIDごとに`record`、`complete`または`discard`の操作種別とwireへ送った完全なrequestを保持する。repository確認後に全体blockを解除しても未確定taskの固定requestは残し、そのtaskでは同じ操作の同じrequestだけを再送できる。旧schemaの`discard_event_ids`または`discard_event_ended_at_epoch_ms`を含むmarkerは、完全なpayloadを復元できないため安全側の手動確認blockとして読み込む。未知version、JSON不正、schema不正、重複・対応requestなし・全体block解除済みのcommit済みIDでは、固定requestとcommit済みIDの双方を信頼せず破棄し、mutation blockedとして復元する。
...
専用workerは既存5 commandに`discard_session`と`list_discarded_sessions`を加えた7 commandを順番に処理する。workerへの送信順が実行順となる。
...
3. `record_elapsed_seconds`が`false`なら追加実績秒を0とし、`started_at_epoch_ms`と終了時刻から1秒以上の`WebDiscardComplete` eventを生成する。1秒未満ならeventを生成しない。
...
`complete_session(record_elapsed_seconds: false)`は同じ入力情報から`WebDiscardComplete` eventを作り、task完了とjournal追記を1 transactionで保存する。1秒未満はeventなしでtaskだけを完了する。`true`ではjournalを変更しない。

### 4.7 `list_discarded_sessions`

`logical_date`を受け、`ServerSnapshot`と、日次合計秒、task別合計、開始時刻順event内訳を返す。event DTOはevent/task UUID、開始時task名、開始・終了epoch milliseconds、経過秒、source、reasonを保持する。共通集計のoverflowは`arithmetic_overflow`へ写像する。
...
- 終了操作をdispatchしたsessionはclick時刻と見積到達時刻の早い方で未送信進捗の加算を打ち切る。未commitが確定するerrorでは対象を計測中へ戻し、transport切断または`repository_state_uncertain`ではrepository確認後も固定requestの確定応答までclick時刻の終端を保持する。完了実績競合では初回click後の壁時計減算を相殺し、確認中と再送中の表示を固定する。成功時はresponseのsnapshotを新たな基準とするため、通信待ち時間を未送信進捗へ加算しない。
...
- 「計測を破棄して解除」はclick時刻で停止し、破棄event UUIDと開始時task名を含む`discard_session`をstorage-first safety marker設定後に送る。server成功後だけlocalStorageとmemoryから削除し、残存sessionからbufferを再計算して一覧を再取得する。
